...
def shell():
    ...
    printer('Procurando o provedor da rede...', quiet)
    try:
        speedtest = Speedtest(
        ...
        printer('Não é possível encontrar o provedor da rede', error=True)
    ...
    if args.list:
        try:
            ...
            printer('Não é possível encontrar a lista de servidores speedtest', error=True)
            raise SpeedtestCLIError(get_exception())
    ...
    printer('\n\033[1;33mProvedor: %(isp)s\033[0m' % speedtest.config['client'],
            quiet)

    if not args.mini:
        printer('\nProcurando lista de servidores de teste...', quiet)
        try:
            ...
            raise SpeedtestCLIError(
                '%s é um tipo de servidor inválido, deve ser um interno' % ', '.join('%s' % s for s in args.server)
            )

        if args.server and len(args.server) == 1:
            printer('Encontrando informações para o servidor selecionado...', quiet)
        else:
            printer('Selecionando o melhor servidor...', quiet)
    ...
    printer('\n\033[1;33mTeste iniciado por: %(sponsor)s (%(name)s)\033[0m'
            '\n\n\033[1;32mPing: %(latency)s ms\033[0m' % results.server, quiet)
            
    if args.download:
        printer('\nTestando velocidade de download', quiet,
                end=('', '\n')[bool(debug)])
        speedtest.download(
            callback=callback,
            threads=(None, 1)[args.single]
        )
        printer('\n\033[1;32mVelocidade de download: %0.2f M%s/s\033[0m' %
                ((results.download / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]),
                quiet)
    else:
        printer('Pulando teste de download', quiet)

    if args.upload:
        printer('\nTestando velocidade de upload', quiet,
                end=('', '\n')[bool(debug)])
        speedtest.upload(
            callback=callback,
        ...
        printer('\n\033[1;32mVelocidade de upload: %0.2f M%s/s\033[0m' %
                ((results.upload / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]),
                quiet)
    else:
        printer('Pulando teste de upload', quiet)
